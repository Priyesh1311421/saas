...
const Header = () => {
    ...
    const [hasScrolled, setHasScrolleed] = useState(false);
    
    const [isOpen, setIsOpen] = useState(false);

    useEffect(()=>{
        const handleScroll = () => {
            setHasScrolleed(scrollY > 32)
        }
    ...
    const Navlink = ({title}:{title:any}) =>(
        <LinkScroll onClick={()=>{setIsOpen(false)}} className='base-bold text-p4 uppercase transition-colors duration-500 cursor-pointer hover:text-p1 max-lg:my-4 max-lg:h-5' offset={-100} to={`${title}`} spy smooth activeClass='nav-active'>
         {title}
        </LinkScroll>
    )
    return (
